program main
  ...
  use regrid_module
  use exact_sol_module
  use compute_error_module
  ...
  type(multifab), allocatable :: phi_exact(:)
  ...
  allocate(phi_exact(max_levs))
  allocate(velocity(max_levs,dim))
  allocate(force(max_levs))
  ...
  do n=1,nlevs
     ...
     call multifab_build(phi_exact(n),mla%la(n),1,0)
     do i=1,dim
        call multifab_build_edge(velocity(n,i),mla%la(n),1,2,i)
     end do
     call multifab_build(force(n),mla%la(n),1,1)
  end do
  ...
  call set_velocity(mla,velocity,dx,time,prob_lo)
  call exact_sol(mla,phi_exact,prob_lo,prob_hi, dx,time)
  ! Write plotfile at t=0
  call write_plotfile(mla,phi_new,velocity,istep,dx,time,prob_lo,prob_hi)
  ...
  do istep=1,nsteps
     ...
     if ( istep > 1 .and. max_levs > 1 .and. regrid_int > 0 .and. &
        ...
        do n = 1,nlevs
          ...
          call multifab_destroy(phi_exact(n))
          do i = 1, dim
