module bds_module
  ...
  public :: bds, bds_single_level, bds_velpred

contains

  subroutine bds(s,sedge,umac,force,dx,dt, &
...
! KBH temporary remove isvel input
...
!    type(bc_level) , intent(in   ) :: the_bc_level(:)
    ...
    do comp=start_scomp,start_scomp+num_comp-1
       do i = 1, nfabs(s)
          ...
          umacp  => dataptr(umac(1), i)
          vmacp  => dataptr(umac(2), i)
          fp     => dataptr(force ,i)
          lo =  lwb(get_box(s, i))
          hi =  upb(get_box(s, i))
          select case (dm)
  ...
  end subroutine bds_single_level
  ...
  subroutine bds_velpred_2d(lo,hi,dx,dt,u,ng_u,uslope,vslope,ng_s,umac,vmac,ng_um,force,ng_f)
    ...
    real(kind=dp_t), intent(in   ) :: dx,dt ! note set dx =dy = dz for now
